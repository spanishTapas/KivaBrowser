...
class AboutKivaBrowser: UIViewController, UITextViewDelegate {
    ...
    func setupAboutKivaBrowser() {
        var aboutKivaBrowser = "KivaBrowser is an iOS client for kiva.org. KivaBrowser makes it easy to explore microfinance via the Kiva API. You can browse the latest lending_actions and loan_requests; browse loans, lending_teams, and lenders by different orders; and filter search results by customized criteria.\n"
        ...
        let bodyStr = NSMutableAttributedString(string: aboutKivaBrowser, attributes: bodyAttr)
    ...
    func setupAboutKivaOrg() {
        ...
        let aboutKiva = "Kiva.org is a non-profit organization with a mission to connect people through lending to alleviate poverty. Leveraging the internet and a worldwide network of microfinance institutions, Kiva lets individuals lend as little as $25 to help create opportunity around the world."
        let headerAttr: Dictionary = [NSFontAttributeName: UIFont.boldSystemFontOfSize(15)]
        let headerStr = NSMutableAttributedString(string: "About Kiva.org\n\n", attributes: headerAttr)
        let bodyAttr: Dictionary = [NSFontAttributeName: UIFont.systemFontOfSize(14), NSForegroundColorAttributeName: UIColor.darkGrayColor()]
        let bodyStr = NSMutableAttributedString(string: aboutKiva, attributes: bodyAttr)
        headerStr.appendAttributedString(bodyStr)
